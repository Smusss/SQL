...
INSERT schedule (train_id, station, station_time)
VALUES
...
DROP VIEW IF EXISTS friend_of_1;
...
DROP VIEW IF EXISTS messages_with_1;
CREATE VIEW messages_with_1 AS
(SELECT body FROM messages WHERE from_user_id = 1 OR to_user_id = 1);
SELECT * FROM messages_with_1;


-- Получите список медиафайлов пользователя с количеством лайков(media m, likes l ,users u)

SELECT m.id as media_id, m.filename AS file_name, CONCAT(u.firstname, ' ', u.lastname) AS full_name, likes_num.count_likes
FROM media AS m
LEFT JOIN users AS u 
ON m.user_id = u.id
LEFT JOIN 
	(
	SELECT media_id, COUNT(media_id) AS count_likes 
	FROM likes
	GROUP BY media_id
    ) AS likes_num
ON m.id = likes_num.media_id
WHERE count_likes IS NOT NULL;


-- Получите количество групп у пользователей
SELECT CONCAT(u.firstname, ' ', u.lastname) AS full_name, comm_num.community_num
FROM users AS u
RIGHT JOIN 
(
SELECT user_id, COUNT(community_id) AS community_num
FROM users_communities AS uc
GROUP BY user_id
) AS comm_num
ON u.id = comm_num.user_id; 
