...
DROP DATABASE IF EXISTS homework_3_add;
CREATE DATABASE homework_3_add;
...
-- Доп по базе данных для ВК( https://www.notion.so/c448e32ae1344f22b1deae7f42c8b57f ):
-- Посчитать количество документов у каждого пользователя (doc, docx, html)

SELECT users.id, CONCAT(users.firstname, " ", users.lastname) AS fullname, COUNT(media.media_type_id) AS num_media
FROM users, media
WHERE  media.user_id = users.id AND media.media_type_id = 4
GROUP BY users.id
ORDER BY users.id;
...
SELECT users.id, CONCAT(users.firstname, " ", users.lastname) AS fullname, media.filename, COUNT(likes.id) AS num_likes
FROM users, media, likes
WHERE likes.user_id = users.id AND likes.media_id = media.id AND media.media_type_id = 4
GROUP BY users.id, media.id;

SELECT COUNT(*) FROM likes, media, users
WHERE likes.user_id = users.id AND likes.media_id = media.id AND media.media_type_id = 4;