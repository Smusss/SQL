USE homework_02;
DROP TABLE IF EXISTS users, posts, clients;
...
SELECT posts.title, posts.full_text, posts.user_id, users.login
FROM posts, users
WHERE posts.user_id = users.id;

-- №3.	Выполните поиск  по публикациям, автором котоырх является клиент "Mikle".
SELECT posts.id, posts.title, posts.full_text, posts.user_id, users.login FROM posts, users
