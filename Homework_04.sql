...
SELECT shops.shopname AS 'В наличии в магазине'
FROM shops
LEFT JOIN cats
...
FROM shops, cats
WHERE shops.id = cats.shops_id AND cats.name != 'Murzik' AND cats.name != 'Zuza';
...
FROM Analysis AS a, Orders AS o
WHERE a.an_id = o.ord_an AND left(o.ord_datetime, 10) = '2020-02-05';
...
FROM likes AS l, users AS u, `profiles` AS p
WHERE l.user_id = u.id AND p.user_id = u.id AND (YEAR(NOW()) - YEAR(p.birthday) < 12);
...
SELECT p.gender, COUNT(l.user_id) AS male_likes
FROM  `profiles` AS p, likes AS l
WHERE l.user_id = p.user_id
GROUP BY p.gender
ORDER BY male_likes DESC
LIMIT 1;
...
SELECT concat(u.firstname, ' ', u.lastname) AS fullname, COUNT(m.from_user_id) AS count_mes
FROM users AS u
...
SELECT CONCAT(u.firstname, ' ' , u.lastname) AS fullname, COUNT(m.to_user_id)
FROM users AS u
RIGHT JOIN messages AS m
ON u.id = m.to_user_id
-- WHERE u.firstname = 'Reuben'
GROUP BY fullname
ORDER BY fullname;
-- LIMIT 1;

SELECT CONCAT(u.firstname, ' ' , u.lastname) AS from_user, m.to_user_id, m.from_user_id
FROM users AS u
RIGHT JOIN messages AS m
ON u.id = m.from_user_id;
